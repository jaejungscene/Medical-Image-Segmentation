...
from .multiwayblock import MultiWayBlock
...
class Encoder(nn.Module):
    def __init__(self, config, vis):
        ...
        for _ in range(config.transformer["num_layers"]):
            if config.encoderType=="normal":
                layer = Block(config, vis)
            elif config.encoderType=="multiway": # <--------------------------------- multiway transformer
                layer = MultiWayBlock(config, Block)
...
class TransUnet(nn.Module):
    ...
    def forward(self, x):
        if x.size()[1] == 1:
            x = x.repeat(1,3,1,1)
        x, attn_weights, features = self.transformer(x)  # x: (B, n_patch, hidden)
