...
class PVT_CASCADE(nn.Module):
    ...
    def load_pvt_weights_from(self, weights):
...
class TransCASCADE(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        channels = [768,512,256,64] if self.encoderType=="normal"\
              else [config.hidden_size,512,256,64]
        ...
        # prediction heads
        for i, in_ch in enumerate(channels):
            self.add_module(
                f"segmentation_head{i+1}",
                SegmentationHead(
                    in_channels=in_ch,
