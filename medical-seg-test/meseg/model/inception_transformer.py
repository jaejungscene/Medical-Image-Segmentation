...
class Mlp(nn.Module):
    def __init__(self, in_features:int, hidden_features:int, out_features:int,
                 act_layer=nn.GELU, bias:bool=True, drop:float=0.) -> None:
...
class InceptionBlock(nn.Module):
    ...
    def __init__(self, dim:int, num_heads:int, mlp_ratio:float=4.,
                 qkv_bias:bool=False, num_path:int=1, pixshuf_factor:int=1,
                 attn_drop:float=0., proj_drop:float=0., act_layer=nn.GELU,
...
class InceptionTransformer(nn.Module):
    def __init__(self, dim:int, num_heads:int, img_size:int=224, mlp_ratio:float=4.,
                 qkv_bias:bool=False, num_path:int=1, pixshuf_factor:int=1,
