...
from meseg.dataset import btcv_2d_inference, acdc_2d_validation
...
def train_one_epoch_with_valid(
    ...
    # 5. each epoch last validation
    if args.valid_freq==None  or  args.valid_freq==0:
        validate(valid_dataloader, model, criterion, args, epoch=epoch, train_loss=loss)
    elif args.dataset_type == "btcv_2d":
        performance = 0
        if epoch+1 == args.epoch or (epoch>30 and (epoch+1)%10==0):
            args.log("="*100)
            performance = btcv_2d_inference(valid_dataloader, args, model)
            args.log("="*100)
        if args.use_wandb:
            wandb.log({"avg tarin loss":loss})
        if performance > args.best:
            args.best = performance
            args.log(">>>>> best model is changed <<<<<")
            torch.save(model.state_dict(), args.best_weight_path)
    elif args.dataset_type == "acdc_2d":
        acdc_2d_validation(valid_dataloader, model, args)
    else:
        assert False, f"{args.dataset_type} is not supported yet about validation"
    args.log("*"*100)
    return global_step